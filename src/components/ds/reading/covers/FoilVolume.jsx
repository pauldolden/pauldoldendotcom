...
/**
 * FoilVolume — the story as a stylised bound hardback: a `coverColor` cloth
 * face, a darkened spine with raised cords, a double-rule foil frame with
 * corner studs, and a stamped emblem cartouche holding the title monogram +
 * "VOL · N". Deterministic from a hash of (id || title): SSR-safe, and every
 * story differs even when several share one `coverColor`. Fills any aspect
 * ratio; the parent draws the badge + bottom scrim + title on top.
 */
...
export function FoilVolume({ title, id, tags = [], coverColor = 'var(--accent)', status }) {
